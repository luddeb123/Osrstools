    ...
    <v-form v-model="valid">
      <v-form style="min-width:250px; display:flex;">
        <v-text-field
          v-model="KCtoFind"
          :rules="alwaysReq"
          label="What killcount do you want to update or delete?"
          required
          type="number"
        ></v-text-field>
      </v-form>
      <v-btn block color="primary" @click="findThisKillcount">Find the data from this killcount</v-btn>
    </v-form>
    <v-form>
      <v-form v-model="valid">
        <v-form style="min-width:250px; display:flex;">
      ...
      <v-btn block color="primary" @click="updateThisKillcount">Update to the new values</v-btn>
      <v-divider />
      <v-btn
        :disabled="ValidFormRequest"
        block
        color="error"
        @click="removeFromLootlogger"
      >Remove this KC</v-btn>
    </v-form>
  </div>
</template>
...
export default {
  ...
  methods: {
    findThisKillcount() {
      this.$store.dispatch("FindThisRaidData", this.KCtoFind);
    },

    autoFillFields() {
      let data = this.$store.getters.dataOfRaidBeingEdited;
      this.gotPet = null + data.Pet;
      this.KCToUpdate = null + data.KC;
      this.raidPoints = null + data.Points;
      this.SpecialLoot = null + data.Unique;
      this.EliteCluescroll = null + data.Clue;
      if (data.Unique) {
        this.SpecialLootID = null + data.Loot[0].Item;
        ...
        this.LootOneItem = null + data.Loot[0].Item;
        this.LootOneAmount = null + data.Loot[0].Amount;
        this.LootTwoItem = null + data.Loot[1].Item;
        this.LootTwoAmount = null + data.Loot[1].Amount;
      }
    },

    removeFromLootlogger() {
      this.$store.dispatch("RemoveThisFromLootlog", this.KCToUpdate)
    },

    updateThisKillcount() {
      if (this.SpecialLoot) {
        if (this.KCToUpdate && this.raidPoints && this.SpecialLootID != null) {
          this.valid = true;
        }
      }
      let payload = {};
      ...
      this.$store.dispatch("updateThisRaid", payload);

      this.KCToUpdate = null;
  ...
  computed: {
    ...
    dataFromStore() {
      return this.$store.getters.dataOfRaidBeingEdited;
    }
  },
