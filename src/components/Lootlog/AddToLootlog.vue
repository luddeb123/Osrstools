    ...
    <v-form>
      <v-form v-model="valid">
        <v-form style="min-width:250px; display:flex;">
          ...
          ></v-text-field>
          <v-switch
            v-model="getKCFromPrevousKC"
            @change="KCFromPrevousKC"
            label="Predict KC"
            color="#1976D2"
          ></v-switch>
        </v-form>
...
export default {
  ...
  methods: {
    ...
    KCFromPrevousKC() {
      if (this.getKCFromPrevousKC) {
        let highestKC = Number(this.$store.state.lootlog.highestKC);
        let nextKC = (highestKC + 1);
        this.raidKillcount = String(nextKC);
      } else {
        this.raidKillcount = this.raidKillcount;
      }
    }
  },
  computed: {
